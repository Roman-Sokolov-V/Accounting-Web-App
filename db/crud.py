...
def get_partner(db: Session, partner_id: int|None = None):
    if partner_id is not None:
        partner = db.query(DBPartners).filter(
            DBPartners.id == partner_id
        ).first()
        return partner
    else:
...
def get_partner_entries(db: Session, partner_id: int|None = None):
    if partner_id is not None:
        entries = (
            ...
            .options(
                joinedload(DBEntries.account),
                joinedload(DBEntries.transaction)
            )
...
def get_all_transactions(db: Session):
    return db.query(DBTransactions).options(
        joinedload(DBTransactions.entries)
    ).all()
...
def get_all_entries(db: Session):
    return db.query(DBEntries).options(
        joinedload(DBEntries.account)
    ).all()
